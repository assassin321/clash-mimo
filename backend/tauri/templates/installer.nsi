...
;
; Keep it in sync with upstream, but preserve the Clash Mimo customizations.
; They are all marked with "Clash Mimo:" comments throughout this file:
...
Var ProgramDataPathVar ; Clash Mimo: ProgramData path for nyanpasu-service
...
Function PageReinstall
  ...
  ${Else}
    ...
    ;
    ; Clash Mimo: on first entry, default to the second radio "don't uninstall"
...
; ===== Clash Mimo customizations: macros =====

; Clash Mimo: resolve the ProgramData folder path into $ProgramDataPathVar.
...
; Clash Mimo: stop and (optionally) kill a single nyanpasu-related process.
...
; Clash Mimo: stop and kill all nyanpasu-related processes (main binary + cores).
!macro CheckAllNyanpasuProcesses
  !insertmacro CheckNyanpasuProcess "Clash Mimo.exe" "1"
...
; Clash Mimo: ask the running nyanpasu-service to stop the core before we replace files.
...
; Clash Mimo: uninstall nyanpasu-service and remove its data directory.
...
; Clash Mimo: clean up autostart entries and custom protocol handlers.
!macro RemoveRegs
  ; cleanup auto start registry keys
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "Clash Mimo"
  DeleteRegValue HKLM "SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Run" "Clash Mimo"
...
; ===== End Clash Mimo customizations: macros =====
...
Section Install
  ...
  ; Clash Mimo: gracefully stop the running core via the service, then stop core processes
...
Section Uninstall
  ...
  ; Clash Mimo: stop core processes before removing files
  !insertmacro CheckAllNyanpasuProcesses
  ...
  ; Clash Mimo: split cleanup into "system integration" and "user data".
  ;
  ...
  ; integrations intact.
  ${If} $UpdateMode <> 1
    ; Clash Mimo: stop and remove nyanpasu-service and its data directory
    !insertmacro GetProgramDataPath
    !insertmacro StopAndRemoveServiceDirectory

    ; Clash Mimo: remove autostart entries and clash / clash-nyanpasu protocol handlers
  ...
  ${AndIf} $UpdateMode <> 1
    ...
    ; Clash Mimo: also remove legacy app data stored under the product name
    RmDir /r "$APPDATA\Clash Mimo"
    RmDir /r "$LOCALAPPDATA\Clash Mimo"
  ${EndIf}
