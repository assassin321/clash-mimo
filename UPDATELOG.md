...
> [!IMPORTANT]
>
> 重构了 Clash Mimo Service 构建，不再提供 `install-service` 和 `uninstall-service` 二进制文件，仅使用 `clash-verge-service` 二进制文件来完成
>
> 优化了 Clash Mimo Service 服务重启核心的逻辑，需要手动重新安装服务模式 (`设置` -> `服务模式` -> `卸载` -> `安装`)
>
...
- 增强日志界面数据渲染的性能
- 优化服务模式下的内核重启逻辑
- 优化 Clash Mimo Service 服务的安装逻辑，可直接覆盖旧的服务
...
- 优化更新了 Clash Mimo Service 服务，需要在设置界面中重新卸载安装服务
...
- 与 Clash Mimo Service 的通信改用 IPC 通信，**需要卸载旧版本服务**（Macos 未验证）
...
> [!Important]
>
> `1.7.x` 版本重构了 Clash Mimo Service 一部分功能以及更改了应用的 appid、identify 以及 aur 构建包名，升级到 `1.7.x` 版本前，请先卸载旧版本的服务模式并自行备份订阅配置文件
...
- `pnpm check` 支持 `--alpha` 参数来下载 alpha 版本的 Clash Mimo Service
...
- 修改 Clash Mimo Service 的 API 调用为 RESTful API 的风格
