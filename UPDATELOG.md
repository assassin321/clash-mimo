...
> [!IMPORTANT]
>
> **Read Before Upgrading: Version 1.6.7 Requires Uninstalling Previous Versions and Performing a Fresh Install** 

> ### ✨ Features
- Optimized the width of the delete confirmation popup
- Added a timeout mechanism for requests to the Mihomo API
- Improved the delay test method for proxy groups, added a parameter to decide whether to clear fixed nodes
- Optimized macOS desktop icons
- Improved logic for restoring proxy settings when the system shuts down or restarts
- On macOS, when using tun, the name must follow the `utun + number` format
- Optimized display of target addresses in connection info
- Refresh frontend proxy info after successful app backup
- Support for proxy settings with dial-up/VPN connections (experimental)
- Improved Monaco editor initialization and configuration, optimizing loading and rendering experience
- Adjusted `Show in Dock` setting behavior for whether the app remains in the dock when closing windows
- Frontend state management migrated to Zustand to improve stability and maintainability
- Optimized page transitions and route preloading to enhance smoothness and first-screen experience
- Optimized core operation and restart logic
- Added in-app shortcut settings: macOS defaults to `⌘ + Q` to quit, other platforms default to `Esc` to close windows
- Optimized script log display interface
- Local backup supports custom backup paths
- Local backup supports displaying a list of backup files
- Optimized rule page rendering speed
- Support for enabling/disabling rules
- Support for toggling themes by clicking sidebar app icons
- Fully manage the path configuration of rule sets (default saved in the `rules` folder in the app directory)
- Rebuilt proxy group page to enable sticky scrolling
- Rule page supports fuzzy content search / domain search / CIDR block search
- Added `Back to Top` button in the connection interface
- Optimized proxy group icon caching logic
- Labels for proxy nodes now show the provider of the proxy set
- Connection interface supports field filtering
- Optimized the style of Fixed nodes (using warn theme color for visibility)
- Support for setting background color
- Reset default theme color
- Improved theme color picker
- Subscription interface supports multi-select operations
- Redesigned and optimized interface themes and styles based on Material You design language

### 🐛 Bug Fixes

- Fixed node latency data color display and style errors
- Fixed transparent background issue in connection table header area
- Fixed background transparency issue for active subscriptions/scripts when dragging on the subscription page
- Fixed issue where dragging a subscription item without changing the order would incorrectly trigger selection and switch subscriptions
- Disabled theme animations on Windows to prevent flickering when switching themes
- Simplified system tray tooltip information on Windows to prevent incomplete display
- Fixed incorrect calculation of node column display on the proxy interface
- macOS: unable to restore window from Dock
- macOS: incorrect system tray icon color
- On app startup, two app icons appear in the macOS Dock
- Fixed window size hook initializing to 0 to avoid abnormal window dimensions
- macOS: flickering issue when switching tray icons
- Fixed issue where paused logs in the log interface couldn't be cleared
- macOS: constantly switching configurations causes memory usage to keep rising

---
### 🔧 Refactor

- Refactored logic for parsing Mihomo IPC data
- Refactored Mihomo IPC handling to improve communication stability and readability
- Refactored i18n language files
- Refactored service mode
- Refactored connection list using tanstack table
- Removed custom error implementation
- Integrated Clash Mimo Service
...
- 全新专属 Mimo rev UI 界面 (by @Amnesiash) 及细节调整
- 提供允许无效证书的开关
- 删除不必要的快捷键
- Provider 更新添加动画
- Merge 支持 Provider
