...
const SettingMimo = ({ onError }: Props) => {
  ...
  const pendingViewerRef = useRef<MimoViewerKey | null>(null);
  const [mountedViewers, setMountedViewers] = useState<
    Partial<Record<MimoViewerKey, boolean>>
  >({});
  ...
  const handleSelectLocalBackupDir = async () => {
    ...
    if (selected) {
      ...
      setSaving(true);
      try {
        await patchMimo({ local_backup_dir: nextDir });
      } finally {
        setSaving(false);
      }
    }
  ...
  const handleResetLocalBackupDir = async () => {
    ...
    setSaving(true);
    try {
      await patchMimo({ local_backup_dir: "" });
    } finally {
      setSaving(false);
    }
  };
    ...
    <SettingList title={t("pages.settings.verge.title")}>
      ...
      <SettingItem label={t("pages.settings.verge.misc.appLogLevel")}>
        <GuardState
          ...
          onGuard={(e) => patchMimo({ app_log_level: e })}>
      ...
      <SettingItem label={t("common.fields.language")}>
        <GuardState
          ...
          onCatch={onError}
          onFormat={(e: any) => e.target.value}
          onGuard={(e) => patchMimo({ language: e })}>
          <Select size="small" sx={{ width: 110, "> div": { py: "7.5px" } }}>
      ...
      <SettingItem label={t("pages.settings.verge.themeMode.label")}>
        <GuardState
          value={themeMode}
          onCatch={onError}
          onGuard={(e) => patchMimo({ theme_mode: e })}>
          <ThemeModeSwitch />
        </GuardState>
      </SettingItem>
      ...
      {OS !== "linux" && (
        <SettingItem label={t("pages.settings.verge.tray.clickEvent")}>
          <GuardState
            ...
            onCatch={onError}
            onFormat={(e: any) => e.target.value}
            onGuard={(e) => patchMimo({ tray_event: e })}>
      ...
      <SettingItem
        ...
        <GuardState
          ...
          onCatch={onError}
          onFormat={(e: any) => e.target.value}
          onGuard={(e) => patchMimo({ env_type: e })}>
          <Select size="small" sx={{ width: 140, "> div": { py: "7.5px" } }}>
      ...
      <SettingItem label={t("common.fields.startPage")}>
        <GuardState
          ...
          onCatch={onError}
          onFormat={(e: any) => e.target.value}
          onGuard={(e) => patchMimo({ start_page: e })}>
          <Select size="small" sx={{ width: 140, "> div": { py: "7.5px" } }}>
            {routes.map((route) => {
              return (
      ...
      <SettingItem label={t("common.fields.startupScript")}>
        <GuardState
          ...
          onCatch={onError}
          onFormat={(e: any) => e.target.value}
          onGuard={(e) => patchMimo({ startup_script: e })}>
          <Input
            value={startupScript}
            disabled
            sx={{ width: 230 }}
              ...
              <>
                <Button
                  onClick={async () => {
                    ...
                    if (path?.length) {
                      await patchMimo({ startup_script: `${path}` });
                    }
                  }}>
                ...
                {startupScript && (
                  <Button
                    onClick={async () => {
                      await patchMimo({ startup_script: "" });
                    }}>
