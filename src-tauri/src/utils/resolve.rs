...
pub fn setup_panic_hook() {
    std::panic::set_hook(Box::new(move |panic_info| {
        ...
        let app_log_file = get_relative_path(MimoLog::global().get_app_log_file());
        let clash_log_file = get_relative_path(MimoLog::global().get_clash_log_file());
                ...
                if let Ok(item) = PrfItem::from_url(url, None, None, Some(option)).await {
                    if let Ok(restart_core_) = Config::profiles().latest_mut().append_item(item) {
                        if handle::Handle::get_window().is_some() {
                            ...
                            handle::Handle::notify("Clash Mimo", t!("notice.import.success"));
                        }
                    ...
                    if handle::Handle::get_window().is_some() {
                        ...
                        handle::Handle::notify("Clash Mimo", t!("notice.import.failed"));
                    }
