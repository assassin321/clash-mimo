...
**Clash Mimo** — A [Tauri v2](https://v2.tauri.app/) desktop GUI for [Mihomo](https://github.com/MetaCubeX/mihomo) (Clash Meta) proxy client. Forked from Clash Mimo Rev v1.6.0.
...
| `clash-mimo-service` | Standalone sidecar binary that manages the Mihomo core process                  |
| `clash-mimo-utils`   | Shared utility functions                                                        |

### Data Flow

```
User UI (React) → Tauri IPC commands (cmds/) → Core services (core/) → Mihomo REST API (tauri-plugin-mihomo)
                                                      ↘ Sidecar service (clash-mimo-service) → Mihomo core
```
...
This project is indexed by GitNexus as **clash-mimo** (6410 symbols, 11418 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource                                          | Use for                                  |
| ------------------------------------------------- | ---------------------------------------- |
| `gitnexus://repo/clash-mimo/context`        | Codebase overview, check index freshness |
| `gitnexus://repo/clash-mimo/clusters`       | All functional areas                     |
| `gitnexus://repo/clash-mimo/processes`      | All execution flows                      |
| `gitnexus://repo/clash-mimo/process/{name}` | Step-by-step execution trace             |
