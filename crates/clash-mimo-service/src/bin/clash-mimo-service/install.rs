use clash_mimo_service::{DEFAULT_SERVER_ID, Result, ServiceError};
...
pub fn process(server_id: Option<String>) -> Result<()> {
    ...
    log::debug!("Start install Clash Mimo Service.");

    // uninstall old service
    crate::uninstall::process()?;

    let service_binary_path = std::env::current_exe()?.with_file_name("clash-mimo-service");
    let target_binary_path = "/Library/PrivilegedHelperTools/mimo.clash.app.helper";
    ...
    if !service_binary_path.exists() {
        log::error!("The clash-mimo-service binary not found.");
        std::process::exit(2);
    }
    ...
    let plist_file = "/Library/LaunchDaemons/mimo.clash.app.helper.plist";
    log::debug!("Create plist file at {}", plist_file);
    let plist_file = Path::new(plist_file);
    let plist_file_content = format!(
        include_str!("../../../tmpl/mimo.clash.app.helper.plist"),
        server_id
    );
...
pub fn process(server_id: Option<String>) -> Result<()> {
    ...
    use clash_mimo_service::SERVICE_NAME;
    ...
    let service_binary_path = std::env::current_exe()
        .unwrap()
        .with_file_name("clash-mimo-service");
    if !service_binary_path.exists() {
        log::error!("The clash-mimo-service binary not found.");
        std::process::exit(2);
    }
...
pub fn process(server_id: Option<String>) -> Result<()> {
    ...
    log::debug!("Start installing Clash Mimo Service.");
    ...
    if let Ok(service) = service_manager.open_service("clash_mimo_service", service_access) {
    ...
    let service_binary_path = std::env::current_exe()
        .unwrap()
        .with_file_name("clash-mimo-service.exe");

    if !service_binary_path.exists() {
        log::error!("clash-mimo-service.exe not found");
        std::process::exit(2);
    }

    let service_info = ServiceInfo {
        name: OsString::from("clash_mimo_service"),
        display_name: OsString::from("Clash Mimo Service"),
    ...
    service
        .set_description("Clash Mimo Service helps to launch clash core")
        .map_err(|e| ServiceError::General(e.to_string()))?;
