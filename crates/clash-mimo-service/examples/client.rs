...
async fn main() -> Result<()> {
    ...
    let mut client =
        clash_mimo_service::Client::connect(server_id, Some(clash_mimo_service::DEFAULT_PSK)).await?;
    // check version
    let now = Instant::now();
    for _ in 0..=2000 {
...
async fn start_core(client: &mut clash_mimo_service::Client) -> Result<()> {
    let curr_dir = std::env::current_dir()?;
    let start_core = StartBody {
        core_type: Some("mihomo".into()),
        socket_path: Some("/tmp/mihomo-dev.sock".into()),
        bin_path: "/Applications/Clash Mimo.app/Contents/MacOS/clash-mihomo".into(),
