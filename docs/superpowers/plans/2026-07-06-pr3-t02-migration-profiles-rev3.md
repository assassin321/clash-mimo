...
- 原子写:`write_profiles_atomic(path, mapping, prefix)`(`modules/profiles.rs:183`),头注释 `# Profiles Config for Clash Mimo`。
...
| R15 | `.bak`:实际转换前把当前原文写 `profiles.yaml.bak`(覆盖旧 .bak);no-op 时不写;写回带头注释 `# Profiles Config for Clash Mimo`                                                                                                                                                                                                                                                                                                                                                                     |
...
```rust
    ...
    fn clean_schema_run_writes_bak_and_validated_new_schema() {
        ...
        assert!(raw.starts_with("# Profiles Config for Clash Mimo\n\n"));
...
fn run_clean_schema(ctx: &mut Ctx) -> anyhow::Result<()> {
    ...
    let content = format!("# Profiles Config for Clash Mimo\n\n{body}");
