...
const repoInfo = { owner: 'assassin321', repo: 'clash-mimo' } as const
...
async function main() {
  ...
  if (!nightlyBuild) {
    lines.push(
      `**Clash Mimo ${version} Released!**`,
      '',
      'GitHub Release:',
      `https://github.com/assassin321/clash-mimo/releases/tag/v${version}`,
    )
  } else {
    lines.push(
      `**Clash Mimo Nightly Build \`${GIT_SHORT_HASH}\`**`,
      '',
    ...
    if (WORKFLOW_RUN_ID) {
      lines.push(
        '',
        'GitHub Actions:',
        `https://github.com/assassin321/clash-mimo/actions/runs/${WORKFLOW_RUN_ID}`,
      )
    }
  }
