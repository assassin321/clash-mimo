...
function getClashMimoService(version: Version): ResourceInfo {
  const fileName = `clash-mimo-service-${sidecarHost}${exeSuffix}`;
  const releaseTag = version === "alpha" ? "alpha" : VERGE_SERVICE_VERSION;
  const downloadURL = `https://github.com/assassin321/clash-mimo-service/releases/download/${releaseTag}/${fileName}`;
...
async function resolveClashMimoService(
  ...
  const downloadItem = getClashMimoService(version);

  logger.message(`Download Clash Mimo Service (${label})`);
  await resolveResource(
    {
      file: `clash-mimo-service${exeSuffix}`,
...
function createTasks(version: Version): Task[] {
  return [
    ...createMihomoTask(),
    {
      name: "Download clash-mimo-service",
      func: (logger: TaskLogger) =>
        resolveClashMimoService(version, logger),
      retry: 5,
