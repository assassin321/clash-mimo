...
async function resolve() {
  ...
  const appPathList = [
    join('macos/Clash Mimo.aarch64.app.tar.gz'),
    join('macos/Clash Mimo.aarch64.app.tar.gz.sig'),
  ]
  ...
  fs.copyFileSync(join('macos/Clash Mimo.app.tar.gz'), appPathList[0])
  fs.copyFileSync(join('macos/Clash Mimo.app.tar.gz.sig'), appPathList[1])
  ...
  await uploadAssets(release.id, [
    join(`dmg/Clash Mimo_${version}_aarch64.dmg`),
    ...appPathList,
  ])
}
