...
function mihomo(version: Channel, mihomoVersion: string): BinInfo {
  ...
  const binName = isAlpha ? "clash-mihomo-alpha" : "clash-mihomo";
...
/**
 * chmod 755 for Clash Mimo Service
 */
async function resolveServicePermission(_logger: TaskLogger) {
  const serviceExecutable = `clash-mimo-service${exeSuffix}`;
...
async function downloadClashMimoService(
  channel: Channel,
  logger: TaskLogger,
) {
  const serviceVersion = getClashMimoServiceVersion();
  const label = channel === "alpha" ? "Alpha" : `v${serviceVersion}`;
  if (!serviceVersion) {
    logger.error(
      `Failed to get Clash Mimo Service version for ${label}.`,
    );
    throw new Error(
      `Failed to get Clash Mimo Service version for ${label}.`,
    );
  ...
  const fileName = `clash-mimo-service-${sidecarHost}${exeSuffix}`;
  const releaseTag =
    channel === "alpha" ? "service-alpha" : `service-v${serviceVersion}`;
  const downloadURL = `https://github.com/assassin321/clash-mimo/releases/download/${releaseTag}/${fileName}`;

  logger.message(`Download Clash Mimo Service (${label})`);
  await resolveResource(
    { file: `clash-mimo-service${exeSuffix}`, downloadURL },
    logger,
  );
  await resolveServicePermission(logger);
}

async function resolveClashMimoService(
  ...
  if (!RUN_ON_GITHUB_ACTIONS) {
    ...
    await downloadClashMimoService(channel!, logger);
  }
}
...
function createMihomoTask(): Task[] {
  return (["stable", "alpha"] as const).map((version) => {
    const isAlpha = version === "alpha";
    const name = isAlpha ? "clash-mihomo-alpha" : "clash-mihomo";
    const taskName = isAlpha
      ? "Download clash-mihomo-alpha"
      : "Download clash-mihomo";
    const label = isAlpha ? "Alpha" : "Stable";
...
function createTasks(channel: Channel | undefined): Task[] {
  return [
    ...createMihomoTask(),
    {
      name: "Resolve Clash Mimo Service",
      func: (logger: TaskLogger) =>
        resolveClashMimoService(channel, logger),
      retry: 5,
      targetPath: resourcePath(`clash-mimo-service${exeSuffix}`),
    },
...
async function chooseServiceChannel(): Promise<Channel> {
  ...
  const channel = await select({
    message: "Select Clash Mimo Service Download Channel",
    options: [
